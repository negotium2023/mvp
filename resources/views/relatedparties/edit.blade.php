...
@section('tab-content')
    ...
    <div class="table-responsive">
        {{Form::open(['url' => route('relatedparty.update', $related_party), 'method' => 'put','class'=>'mt-3', 'files' => true])}}
        <input type="hidden" name="client_id" value="{{$client->id}}"/>
        <input type="hidden" name="related_party_parent_id" value="{{$related_party_id}}"/>
        <table class="table table-bordered table-sm table-hover">
            <thead class="btn-dark">
            <tr>
                <th colspan="4">Related Party</th>
            </tr>
            </thead>
            <tbody>
            <tr>
                <th>Description</th>
                <td>
                    {{Form::text('first_name', $related_party->description,['class'=>'form-control form-control-sm  col-sm-12'. ($errors->has('first_name') ? ' is-invalid' : ''), 'id' => 'first_name'])}}
            ...
            <div id="list_{{$activity['id']}}" class="list-group-item activity group-{{$activity["grouping"]}}" style="{{($activity["grouped"] != null && $activity["grouping"] <= $max_group ? 'display:table;' : 'display:none;')}}width:100%;">
                ...
                <div style="display:table-cell">
                    ...
                    <small class="text-muted"> [{{$activity['type_display']}}] @if($activity['kpi']==1) <i class="fa fa-asterisk" title="Activity is required for step completion" style="color:#FF0000"></i> @endif</small>
                        ...
                        @if($activity['value']==1) <span class="badge badge-pill badge-secondary" title="Activity is required for step completion">Value</span> @endif
