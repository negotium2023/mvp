...
@section('tab-content')
    <div class="table-responsive">
        {{Form::open(['url' => route('relatedparty.store'), 'method' => 'post','class'=>'mt-3', 'files' => true])}}
        <input type="hidden" name="client_id" value="{{$client->id}}"/>
        <input type="hidden" name="related_party_parent_id" value="{{$related_party_id}}"/>
        <input type="hidden" name="step_id" value="{{$step_level_id}}"/>
        <table class="table table-bordered table-sm table-hover">
            <thead class="btn-dark">
                <tr>
                    <th colspan="4">Add Related Party</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <th>First Name</th>
                    <td>
                        {{Form::text('first_name', old('first_name'),['class'=>'form-control form-control-sm  col-sm-12'. ($errors->has('first_name') ? ' is-invalid' : ''), 'id' => 'first_name'])}}
                    ...
                    <th>Last Name</th>
                    <td>
                        {{Form::text('last_name', old('last_name'),['class'=>'form-control form-control-sm  col-sm-12'. ($errors->has('last_name') ? ' is-invalid' : ''), 'id' => 'last_name'])}}
                ...
                <tr>
                    <th>From Existing Client</th>
                    <td>
                        {{Form::select('related_party_id',$clients_drop_down, old('related_party_id'),['class'=>'form-control form-control-sm  col-sm-12'. ($errors->has('related_party_id') ? ' is-invalid' : ''), 'id' => 'related_party_id'])}}
                        @foreach($errors->get('related_party_id') as $error)
                            <div class="invalid-feedback">
                                {{$error}}
                            </div>
                        @endforeach
                    ...
                    <td>
                        {{Form::select('step_id_disabled',$process_activities_drop_down, $step_level_id,['class'=>'form-control form-control-sm  col-sm-12'. ($errors->has('step_id') ? ' is-invalid' : ''), 'id' => 'step_id_disabled', 'disabled' => 'disabled'])}}
                        @foreach($errors->get('step_id_disabled') as $error)
                            <div class="invalid-feedback">
                                {{$error}}
                            </div>
                        @endforeach
