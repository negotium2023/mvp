            ...
            @if(isset($client['rp']) && count($client['rp']) > 0)
                @foreach($client['rp'] as $rp)
                    <tr class="bg-gray-light">
                        ...
                        <td><a href="{{route('relatedparty.show',['client_id' => $rp["client_id"],'process_id' => $rp["process"],'step_id' => $rp["step"],'related_party_id'=>$rp["id"]])}}">@if($rp['consultant'] != null){{$rp['consultant']}} @endif</a></td>
                    </tr>
                @endforeach
            @endif
        @empty
